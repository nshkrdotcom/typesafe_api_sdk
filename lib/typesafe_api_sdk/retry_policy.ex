defmodule TypeSafeAPISDK.RetryPolicy do
  @moduledoc """
  ...
  @spec new(keyword() | map()) :: {:ok, t()} | {:error, TypeSafeAPISDK.Error.t()}
  def new(opts \\ []) do
    ...
    case validate(policy) do
      :ok -> {:ok, policy}
      {:error, message} -> {:error, TypeSafeAPISDK.Error.configuration(message)}
    end
  rescue
    KeyError -> {:error, TypeSafeAPISDK.Error.configuration("unknown retry policy option")}
  end
  ...
  @spec merge(t() | false | nil, t() | keyword() | map() | false | nil) ::
          {:ok, t() | false} | {:error, TypeSafeAPISDK.Error.t()}
  ...
  def merge(_base, _override),
    do: {:error, TypeSafeAPISDK.Error.configuration("invalid retry override")}
  ...
  defp override_attrs(attrs) when is_list(attrs) do
    cond do
      not Keyword.keyword?(attrs) ->
        {:error, TypeSafeAPISDK.Error.configuration("retry override must be a keyword list")}

      length(Keyword.keys(attrs)) != length(Enum.uniq(Keyword.keys(attrs))) ->
        {:error,
         TypeSafeAPISDK.Error.configuration("duplicate retry override options are not allowed")}
