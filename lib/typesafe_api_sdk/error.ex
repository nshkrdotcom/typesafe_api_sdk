defmodule TypeSafeAPISDK.Error do
  @moduledoc """
  Normalized errors from the TypeSafe API and its Pristine transport.

  HTTP status mapping follows the TypeSafe client contract: 400, 401, 403, 404,
  422, 429, and 5xx receive dedicated error types. Transport, cancellation, and
  response-decoding failures remain distinct.
  """
  ...
  def configuration(message) when is_binary(message),
    do: %__MODULE__{type: :configuration, message: message}
  ...
  def response_validation(path, body) when is_list(path) do
    %__MODULE__{
      ...
      message: "Invalid response data at #{inspect(Enum.join(path, "."))}",
      field_path: Enum.join(path, "."),
      path: path,
      body: body
    }
  ...
  def metadata(%__MODULE__{} = error) do
    %{
      type: error.type,
      status: error.status,
      request_id: short_identifier(error.request_id),
      retry_after_ms: error.retry_after_ms,
      retryable: retryable?(error),
      cancelled: error.type == :cancelled,
      field_path: short_identifier(error.field_path)
    }
  end

  @spec retryable?(t(), RetryPolicy.t() | false) :: boolean()
  def retryable?(error, policy \\ %RetryPolicy{})
  def retryable?(_error, false), do: false

  def retryable?(%__MODULE__{type: :connection}, %RetryPolicy{} = policy),
    do: policy.api_connection_error

  def retryable?(%__MODULE__{type: :timeout}, %RetryPolicy{} = policy), do: policy.api_timeout_error

  def retryable?(%__MODULE__{status: status, type: type}, %RetryPolicy{} = policy)
      when is_integer(status) and type not in [:response_validation, :invalid_request],
      do: MapSet.member?(policy.http_statuses, status)

  def retryable?(%__MODULE__{}, _policy), do: false

  @spec retry_after(t()) :: non_neg_integer() | nil
  def retry_after(%__MODULE__{retry_after_ms: retry_after_ms}), do: retry_after_ms
  ...
  def attach_response(%__MODULE__{} = error, %Pristine.Response{} = response, decoded_body) do
    ...
    detail =
      if error.type == :response_validation,
        do: "Invalid response data at #{inspect(error.field_path)}.",
        else: error.message
  ...
  def connection_error(reason, _opts \\ []) do
    if TypeSafeAPISDK.TransportError.timeout?(reason) do
      timeout_error()
    else
      %__MODULE__{
        type: :connection,
  ...
  @doc false
  def timeout_error, do: %__MODULE__{type: :timeout, message: "Request timed out"}
  ...
  defp short_identifier(nil), do: nil
  defp short_identifier(value) when is_binary(value) and byte_size(value) <= 128, do: value
  defp short_identifier(value) when is_binary(value), do: String.slice(value, 0, 128)
  defp short_identifier(value) when is_atom(value) or is_integer(value), do: to_string(value)
  defp short_identifier(_value), do: "<invalid>"
  ...
  defp known_atom_key(_key), do: :__typesafe_api_missing_key__
