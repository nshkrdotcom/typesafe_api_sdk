defmodule TypeSafeAPISDK.SystemOneResponse do
  @moduledoc """
  ...
  alias TypeSafeAPISDK.{ChoiceAnswer, Error, NoulAnswer, ScoreAnswer, TransportResponse, Usage}
  ...
  @doc "Fetch an answer by its wire question ID."
  @spec fetch(t(), String.t()) :: {:ok, answer()} | :error
    ...
    case fetch(response, id) do
      ...
      :error ->
        raise KeyError,
          ...
          term: answers,
          message:
            "no TypeSafe API answer for #{inspect(id)}; " <>
              "available IDs: #{inspect(Enum.sort(Map.keys(answers)))}"
    end
  end
  ...
  defp decode_answer(name, raw) when is_map(raw) do
    case value(raw, "type") do
      ...
      type when is_binary(type) ->
        Logger.warning(
          "Ignoring a TypeSafe API answer with an unrecognized future type; inspect response.raw explicitly"
        )
